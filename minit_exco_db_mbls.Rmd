...
lst <- tibble("Siri"=unlist(minit$Kembaran$Siri),"Perkara"=unlist(minit$Kembaran$Tajuk),"Keterangan"=unlist(minit$Kembaran$Keterangan))
paste(minit$Kembaran$Siri[1], minit$Kembaran$Tajuk[1])  %>% 
  kbl(col.names = "","html", booktabs = T) %>% 
  kable_styling(full_width = TRUE,position = "left")

paste(minit$Kembaran$Keterangan[1])  %>% 
  kbl(col.names = "","html", booktabs = T) %>% 
  kable_styling(full_width = TRUE,position = "left")
