...
\textbf{\underline{NOTA MESYUARAT MMM SIRI `r minit$Siri` PADA `r toupper(format(as.Date(minit$Tarikh,format = "%d/%m/%y"),"%d %b %Y"))` JAM 1000}}
\newline  
...
```
### AGENDA 2: MENGESAHKAN NOTA MESYUARAT

```{r agenda 2, echo=FALSE}
# agenda 2
tibble::tibble("Siri"="2.","Perkara"="Mengesahkan Nota Mesyuarat","Perbincangan dan Keputusan"=minit$Agenda_2$Keterangan,"Tindakan"=minit$Agenda_2$Keputusan)  %>% 
  kbl(longtable = TRUE) %>% 
  kable_styling(position = "left",full_width = FALSE) %>% 
  column_spec(1,width = "0.5cm") %>% 
  column_spec(2,width = "4cm") %>% 
