...
minit <- db$find('{"Siri": "4/23", "Jenis": "exco"}')
...
ya_lst <- read_sheet(ss, sheet = 1) %>% 
  select(Nama, Singkatan, Jawatan, Hadir) %>% 
  filter(Hadir == "Ya")
td_lst <- read_sheet(ss, sheet = 1) %>% 
  filter(Hadir == "Tidak")
```
...
su <- tibble(Nama="Kol Hamdan Bin Yaccob",Singkatan="KHY",Jawatan="Setiausaha", Hadir="Ya")
ya_lst <- bind_rows(ya_lst, su)

ya_lst %>%
  select(Nama, Singkatan, Jawatan) %>% 
