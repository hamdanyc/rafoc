...
rs %>% 
  mutate(siri = row_number(),nama = stringr::str_to_title(nama)) |> 
  select(siri, no_tentera, nama, pkt, done) |> 
  filter(is.na(done)) |> 
  knitr::kable(col.names = c("Siri","No Ten","Nama","Pkt","Flag"))
...
# Kemaskini RData (Agenda 4: Statistik ahli pd minit mesy AJK)
